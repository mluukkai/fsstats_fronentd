...
class Instructor extends React.Component {
  ...
  componentWillMount() {
    ...
    console.log(`${BASEURL}/${this.props.course}/projects`)
    axios.get(`${BASEURL}/${this.props.course}/projects`, config)
      .then(response => {
  ...
  setTime(id, time) {
    ...
    axios.post(`${BASEURL}/projects/${id}/meeting`, { meeting: time }, config)
      .then(response => {
  ...
  deleteTime(id) {
    ...
    axios.delete(`${BASEURL}/projects/${id}/meeting`, config)
      .then(response => {
        const projects = this.state.projects.filter(p => p._id !== id)
        const changed = this.state.projects.filter(p => p._id === id)[0]
  ...
  setInstructor(id, instructor) {
    ...
    axios.post(`${BASEURL}/projects/${id}/instructor`, { instructor }, config)
      .then(response => {
        const projects = this.state.projects.filter(p => p._id !== id)
        const changed = this.state.projects.filter(p => p._id === id)[0]
  ...
  deleteInstructor(id) {
    ...
    axios.delete(`${BASEURL}/projects/${id}/instructor`, config)
      .then(response => {
        const projects = this.state.projects.filter(p => p._id !== id)
        const changed = this.state.projects.filter(p => p._id === id)[0]
